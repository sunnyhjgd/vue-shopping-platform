        ...
        <div class="InfoWrap">
          <div class="goodsDetail">
            ...
            <div class="support">
              <div class="supportArea">
                ...
                <div class="fixWidth">{{ skuInfo.suport }}</div>
              </div>
              <div class="supportArea">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index1) in spuSaleAttrList" :key="index1">
                <dt class="title">{{ item.saleAttrName }}</dt>
                <dd v-for="(attr, index) in item.spuSaleAttrValueList" :key="index" changepirce="0"
                  :class="{ 'active': attr.isChecked }" @click="changeAttr(attr, index1)">{{ attr.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  name: 'Detail',
  mounted() {
    this.$store.dispatch('detail/reqDetailList', this.$route.params.id)
  },
  data() {
    return {
      num: 1,
      maxStock: 9999, // 默认最大库存为9999
    }
  },
  computed: {
    ...mapGetters('detail', ['categoryView', 'skuInfo', 'spuSaleAttrList']),
    imageList() {
      return this.skuInfo.imgList || []
    }
  },
  components: {
    ImageList,
  ...
  methods: {
    ...
    async addCart() {
      try {
        ...
        let result = await this.$store.dispatch('detail/reqAddOrUpdateCart', { skuId: this.skuInfo.id, num: this.num })
        console.log(result.code)
        // 将对象转换为JSON字符串存储到sessionStorage
