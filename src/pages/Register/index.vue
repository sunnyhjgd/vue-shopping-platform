    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <input placeholder="请输入你的登录密码" v-model="password" name="password"
          v-validate="{ required: true, regex: /^[a-zA-Z0-9]{6,12}$/ }" :class="{ invalid: errors.has('password') }" />
      ...
      <div class="controls">
        <input name="isAgree" type="checkbox" v-model="isAgree" v-validate="{ required: true, isAgree: true }"
          :class="{ invalid: errors.has('isAgree') }" />
        <span>同意协议并注册《尚品汇用户协议》</span>
        <span class="error-msg">{{ errors.first("isAgree") }}</span>
      </div>
      <div class="btn">
        <button @click="register" :disabled="!isAgree">完成注册</button>
      </div>
    </div>
    ...
    async getCode() {
      try {
        ...
        if (phone) {
          ...
          this.code = this.$store.state.user.data.code;
        }
      } catch (error) {
        console.log(error);
      }
